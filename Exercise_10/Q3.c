...
void kruskal(Edge* edges, int numEdges, int numVertices) {
    ...
    int mstWeight = 0;
    printf("Power lines to be installed (Minimum Spanning Tree):\n");
    for (int i = 0; i < numEdges; i++) {
        int u = edges[i].src;
        int v = edges[i].dest;
        int weight = edges[i].weight;
        if (find(ds, u) != find(ds, v)) {
            printf("Power line between town %d and town %d with distance %d\n", u, v, weight);
            mstWeight += weight;
            unionSets(ds, u, v);
        }
    }

    printf("Total length of power lines for Minimum Spanning Tree: %d\n", mstWeight);
    free(ds->parent);
    free(ds->rank);
    free(ds);
}
...
int main() {
    ...
    printf("Enter the number of towns (vertices): ");
    scanf("%d", &numVertices);

    while (1) {
        printf("\nMenu:\n");
        printf("1. Add a new power line between towns\n");
        printf("2. Calculate Minimum Spanning Tree (MST) for power lines\n");
        printf("3. Exit\n");
        ...
        switch (choice) {
            case 1: {
                int src, dest, weight;
                printf("Enter source town index: ");
                scanf("%d", &src);
                printf("Enter destination town index: ");
                scanf("%d", &dest);
                printf("Enter power line distance (weight): ");
                scanf("%d", &weight);
